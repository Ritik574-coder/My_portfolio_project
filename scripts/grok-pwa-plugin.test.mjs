...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  const empty = mkdtempSync(join(tmpdir(), "grok-og-test-"));
  const html =
...
test("does not duplicate the extensions script", () => {
  const empty = mkdtempSync(join(tmpdir(), "grok-og-test-"));
  const ctx = { appName: "Demo", projectId: "proj-123", cwd: empty };
  const once = injectGrokPwaHead("<html><head></head></html>", ctx);
  const twice = injectGrokPwaHead(once, ctx);
  assert.equal(once, twice);
...
test("is idempotent", () => {
  const empty = mkdtempSync(join(tmpdir(), "grok-og-test-"));
  const once = injectGrokPwaHead("<html><head></head></html>", { cwd: empty });
  const twice = injectGrokPwaHead(once, { cwd: empty });
  assert.equal(once, twice);
});
